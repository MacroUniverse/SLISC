...
public:
	// constructors
	VbaseQdoub(); // default constructor
	explicit VbaseQdoub(Long_I N);
	VbaseQdoub(const VbaseQdoub &rhs); // copy constructor
	VbaseQdoub(VbaseQdoub&&); // move constructor

	VbaseQdoub &operator=(const VbaseQdoub &); // copy asignment
	VbaseQdoub &operator=(VbaseQdoub &&); // move asignment
...
inline VbaseQdoub &VbaseQdoub::operator=(const VbaseQdoub &rhs)
{
	resize(rhs.m_N);
	if (m_N > 0)
		memcpy(m_p, rhs.m_p, sizeof(Qdoub)*m_N);
	return *this;
}

inline VbaseQdoub &VbaseQdoub::operator=(VbaseQdoub &&rhs)
{
	if (m_N > 0) delete [] m_p;
	m_p = rhs.m_p;
	m_N = rhs.m_N; rhs.m_N = 0;
...
typedef const VbaseQdoub &VbaseQdoub_I;
typedef VbaseQdoub &VbaseQdoub_O, &VbaseQdoub_IO;
...
public:
	// constructors
	VbaseQcomp(); // default constructor
	explicit VbaseQcomp(Long_I N);
	VbaseQcomp(const VbaseQcomp &rhs); // copy constructor
	VbaseQcomp(VbaseQcomp&&); // move constructor

	VbaseQcomp &operator=(const VbaseQcomp &); // copy asignment
	VbaseQcomp &operator=(VbaseQcomp &&); // move asignment
...
inline VbaseQcomp &VbaseQcomp::operator=(const VbaseQcomp &rhs)
{
	resize(rhs.m_N);
	if (m_N > 0)
		memcpy(m_p, rhs.m_p, sizeof(Qcomp)*m_N);
	return *this;
}

inline VbaseQcomp &VbaseQcomp::operator=(VbaseQcomp &&rhs)
{
	if (m_N > 0) delete [] m_p;
	m_p = rhs.m_p;
	m_N = rhs.m_N; rhs.m_N = 0;
...
typedef const VbaseQcomp &VbaseQcomp_I;
typedef VbaseQcomp &VbaseQcomp_O, &VbaseQcomp_IO;
...
public:
	// constructors
	VbaseQimag(); // default constructor
	explicit VbaseQimag(Long_I N);
	VbaseQimag(const VbaseQimag &rhs); // copy constructor
	VbaseQimag(VbaseQimag&&); // move constructor

	VbaseQimag &operator=(const VbaseQimag &); // copy asignment
	VbaseQimag &operator=(VbaseQimag &&); // move asignment
...
// m_p doesn't need to be initialized, but g++ will complain
inline VbaseQimag::VbaseQimag() : m_p(nullptr), m_N(0) {}

inline VbaseQimag::VbaseQimag(Long_I N)
{
#ifdef SLS_CHECK_SHAPES
	if (N < 0)
		SLS_ERR("size less than 0!");
...
inline VbaseQimag::VbaseQimag(const VbaseQimag &rhs)
{
#ifdef SLS_NO_CPY_CONSTRUCTOR
	SLS_ERR("Copy constructor or move constructor is forbidden!");
#endif
...
inline VbaseQimag &VbaseQimag::operator=(const VbaseQimag &rhs)
{
	resize(rhs.m_N);
	if (m_N > 0)
		memcpy(m_p, rhs.m_p, sizeof(Qimag)*m_N);
	return *this;
}

inline VbaseQimag &VbaseQimag::operator=(VbaseQimag &&rhs)
{
	if (m_N > 0) delete [] m_p;
	m_p = rhs.m_p;
	m_N = rhs.m_N; rhs.m_N = 0;
...
inline const Qimag * VbaseQimag::p() const
{
#ifdef SLS_CHECK_BOUNDS
	if (m_N == 0)
		SLS_ERR("using p() for empty container!");
...
inline Long VbaseQimag::size() const
{
	return m_N;
}

inline void VbaseQimag::resize(Long_I N)
{
	if (N != m_N) {
		if (m_N == 0) { // N != 0
...
inline Qimag &VbaseQimag::operator[](Long_I i)
{
#ifdef SLS_CHECK_BOUNDS
if (i<0 || i>=m_N)
	SLS_ERR("VbaseQimag index (" + num2str(i) + ") out of bounds: size = " + num2str(m_N));
#endif
	return m_p[i];
}

inline const Qimag &VbaseQimag::operator[](Long_I i) const
{
#ifdef SLS_CHECK_BOUNDS
	if (i<0 || i>=m_N)
		SLS_ERR("VbaseQimag index (" + num2str(i) + ") out of bounds: size = " + num2str(m_N));
...
inline const Qimag &VbaseQimag::end() const
{
#ifdef SLS_CHECK_BOUNDS
	if (m_N == 0)
		SLS_ERR("tring to use end() on empty vector!");
...
inline Qimag &VbaseQimag::end(Long_I i)
{
#ifdef SLS_CHECK_BOUNDS
	if (i <= 0 || i > m_N)
		SLS_ERR("end(i) index (i = " + num2str(i) + ") out of bound: size = " + num2str(m_N));
...
inline const Qimag &VbaseQimag::end(Long_I i) const
{
#ifdef SLS_CHECK_BOUNDS
	if (i <= 0 || i > m_N)
		SLS_ERR("end(i) index (i = " + num2str(i) + ") out of bound: size = " + num2str(m_N));
...
inline VbaseQimag::~VbaseQimag()
{
	if (m_N != 0)
		delete[] m_p;
}

typedef const VbaseQimag &VbaseQimag_I;
typedef VbaseQimag &VbaseQimag_O, &VbaseQimag_IO;
