...
inline void copy(Scmat3Qdoub_O v, Qdoub_I s)
{
	if (v.size() > 0) vecset(v.p(), s, v.size());
}
...
inline void copy(SvecQcomp_O v, const std::initializer_list<Qcomp> &v1)
{
...
inline void copy(Scmat3Int_O v, const std::initializer_list<Int> &v1)
{
	const Int *p1 = v1.begin();
	Long N = v1.size();
#ifdef SLS_CHECK_SHAPES
...
inline void copy(Scmat3Llong_O v, const std::initializer_list<Llong> &v1)
{
	const Llong *p1 = v1.begin();
	Long N = v1.size();
#ifdef SLS_CHECK_SHAPES
...
inline void copy(Scmat3Doub_O v, const std::initializer_list<Doub> &v1)
{
	const Doub *p1 = v1.begin();
	Long N = v1.size();
#ifdef SLS_CHECK_SHAPES
...
inline void copy(Scmat3Qdoub_O v, const std::initializer_list<Qdoub> &v1)
{
	const Qdoub *p1 = v1.begin();
	Long N = v1.size();
#ifdef SLS_CHECK_SHAPES
...
inline void copy(Scmat3Comp_O v, const std::initializer_list<Comp> &v1)
{
	const Comp *p1 = v1.begin();
	Long N = v1.size();
#ifdef SLS_CHECK_SHAPES
...
inline void copy(Scmat3Qcomp_O v, const std::initializer_list<Qcomp> &v1)
{
	const Qcomp *p1 = v1.begin();
	Long N = v1.size();
#ifdef SLS_CHECK_SHAPES
...
inline void copy(SvbaseQdoub_O v, SvbaseQdoub_I v1)
{
	if (!v.size()) return;
	assert_same_shape1(v, v1);
	veccpy(v.p(), v1.p(), v.size());
...
inline void copy(SvbaseQdoub_O v, DvecQdoub_I v1)
{
	if (!v.size()) return;
	assert_same_shape1(v, v1);
	veccpy(v.p(), v1.p(), v1.step(), v1.size());
...
inline void copy(SvbaseQcomp_O v, SvbaseQcomp_I v1)
{
	if (!v.size()) return;
	assert_same_shape1(v, v1);
	veccpy(v.p(), v1.p(), v.size());
...
inline void copy(SvbaseQcomp_O v, SvbaseQdoub_I v1)
{
	if (!v.size()) return;
	assert_same_shape1(v, v1);
	veccpy(v.p(), v1.p(), v.size());
...
inline void copy(SvbaseQcomp_O v, DvecQcomp_I v1)
{
	if (!v.size()) return;
	assert_same_shape1(v, v1);
	veccpy(v.p(), v1.p(), v1.step(), v1.size());
...
inline void copy(DvecQcomp_O v, SvbaseQcomp_I v1)
{
	if (!v.size()) return;
	assert_same_shape1(v, v1);
	veccpy(v.p(), v.step(), v1.p(), v1.size());
...
inline void copy(McooQcomp_O v, ScmatQcomp_I v1, Doub_I tol = 0)
{
	assert_same_shape2(v, v1);
	if (v.p() == v1.p())
		SLS_ERR("self copy is forbidden!");
...
inline void copy(ScmatQdoub_O lhs, McooQdoub_I rhs)
{
	assert_same_shape2(lhs, rhs);
	copy(lhs, 0);
	for (Long i = 0; i < rhs.nnz(); ++i) {
...
{
	assert_same_shape2(lhs, rhs);
	const Scmat3Qdoub &c3 = lhs.cmat3();
	copy(c3, 0);
	for (Long k = 0; k < rhs.nnz(); ++k) {
		Long i = rhs.row(k), j = rhs.col(k);
		lhs.ref(i, j) = rhs[k];
...
inline void copy(ScmatQdoub_O a, CbandQdoub_I b)
{
	assert_same_shape2(a, b);
	Long N1 = a.n0(), N2 = a.n1();
	for (Long j = 0; j < N2; ++j) {
...
inline void copy(ScmatQcomp_O a, CbandQcomp_I b)
{
	assert_same_shape2(a, b);
	Long N1 = a.n0(), N2 = a.n1();
	for (Long j = 0; j < N2; ++j) {
...
inline void copy(CbandQcomp_O b, ScmatQcomp_I a)
{
	assert_same_shape2(a, b);
	Long N1 = a.n0(), N2 = a.n1();
	for (Long j = 0; j < N2; ++j) {
...
inline void copy(CbandQdoub_O b, ScmatQdoub_I a)
{
	assert_same_shape2(a, b);
	Long N1 = a.n0(), N2 = a.n1();
	for (Long j = 0; j < N2; ++j) {
