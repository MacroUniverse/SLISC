...
// complex version
// warning: this function differs from my Mathematica definition (FlCode.tex)
inline Qcomp acb_coulombF(Qcomp_I l, Qcomp_I eta, Qcomp_I x)
{
	slong prec = 160; // set precision bit (log10/log2 = 3.322)
	acb_t l1, eta1, x1, F1; arb_t F1_re, F1_im;
	acb_init(l1); acb_init(eta1); acb_init(x1); acb_init(F1);
...
inline Qdoub arb_gamma(Qdoub_I x)
{
	slong prec = 160; // set precision bit (log10/log2 = 3.322)
	Qdoub res = 0;
	arb_t x1, res1;
...
inline Qcomp arb_gamma(Qcomp_I z)
{
	slong prec = 80;
	Qcomp res;
	acb_t z1, res1;
...
inline Qcomp arb_lngamma(Qcomp_I z)
{
	slong prec = 80;
	Qcomp res;
	acb_t z1, res1;
...
inline Qdoub arb_coulomb_sigma(Qdoub_I l, Qdoub_I eta)
{
	if (eta > 0)
		SLS_ERR("are you sure?");
	return imag(arb_lngamma(Qcomp(l + 1, eta)));
}
