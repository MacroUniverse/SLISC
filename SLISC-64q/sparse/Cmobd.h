	...
	: m_data(blk_size, blk_size, Nblk), m_N0((blk_size - 1) * Nblk - 1)
{
	...
	// set the first overlapped element to 0
	if (m_data.size() > 0)
		vecset(m_data.p() + step - 1, Qdoub(0), Nblk - 1, step);
}
	...
	: m_data(blk_size, blk_size, Nblk), m_N0((blk_size - 1) * Nblk - 1)
{
	...
	// set the first overlapped element to 0
	if (m_data.size() > 0)
		vecset(m_data.p() + step - 1, Qcomp(0), Nblk - 1, step);
}
...
void vecset(Qimag *, Qimag_I, Long_I, Long_I);
...
public:
	CmobdQimag();
	CmobdQimag(Long_I blk_size, Long_I Nblk);
	const Qimag &operator[](Long_I i) const;
	Qimag &operator[](Long_I i);
	Long find(Long_I i, Long_I j) const; // return single index to m_data, -1 if out of blocks
	const Qimag * p() const; // not the first element!
	Qimag * p();
	Long n0() const;
	Long n1() const;
	Long size() const;
...
typedef const CmobdQimag &CmobdQimag_I;
typedef CmobdQimag &CmobdQimag_O, &CmobdQimag_IO;

inline CmobdQimag::CmobdQimag() : CmobdQimag(0, 0) {}
	...
	: m_data(blk_size, blk_size, Nblk), m_N0((blk_size - 1) * Nblk - 1)
{
	...
	// set the first overlapped element to 0
	if (m_data.size() > 0)
		vecset(m_data.p() + step - 1, Qimag(0), Nblk - 1, step);
}

inline const Qimag &CmobdQimag::operator[](Long_I i) const
{
	return m_data[i];
}

inline Qimag &CmobdQimag::operator[](Long_I i)
{
	return m_data[i];
}

inline Long CmobdQimag::find(Long_I i1, Long_I i2) const
{
#ifdef SLS_CHECK_BOUNDS
	if (i1 < 0 || i1 >= m_N0 || i2 < 0 || i2 >= m_N0)
		SLS_ERR("out of bound!");
...
inline const Qimag * CmobdQimag::p() const
{
	return m_data.p();
}

inline Qimag * CmobdQimag::p()
{
	return m_data.p();
}

inline Long CmobdQimag::n0() const
{
	return m_N0;
}

inline Long CmobdQimag::n1() const
{
	return m_N0;
}

inline Long CmobdQimag::size() const
{
	return m_N0 * m_N0;
}

inline Long CmobdQimag::nnz() const
{
	Long N = nblk0(), Nblk = nblk();
	return (N*N - 1)*Nblk - 4 * N + 3;
}
...
inline Cmat3Qimag& CmobdQimag::cmat3()
{
	return m_data;
}

inline void CmobdQimag::set(Cmat3Qimag_I a3)
{
	veccpy(m_data.p(), a3.p(), a3.size());
	// set the first overlapped element to 0
	Long step = sqr(nblk0());
...
inline Long CmobdQimag::nblk0() const
{
	return m_data.n0();
}

inline Long CmobdQimag::nblk() const
{
	return m_data.n2();
}

inline Qimag CmobdQimag::operator()(Long_I i1, Long_I i2) const
{
	Long ind0 = find(i1, i2);
	if (ind0 >= 0)
		return m_data[ind0];
...
inline Qimag& CmobdQimag::ref(Long_I i, Long_I j)
{
	Long ind = find(i, j);
#ifdef SLS_CHECK_BOUNDS
	if (ind < 0)
...
inline void CmobdQimag::resize(Long_I blk_size, Long_I Nblk)
{
	m_data.resize(blk_size, blk_size, Nblk);
	m_N0 = (blk_size - 1) * Nblk - 1;
	// set the first overlapped element to 0
