...
void veccpy(Qdoub *, const Qdoub *, Long);

class McooQdoub : public VbaseQdoub
{
private:
...
#endif
	if (m_Nnz == m_N)
	    SLS_ERR("McooQdoub::push(): out of memory, please reserve! m_N = " + to_string(m_N));
	m_p[m_Nnz] = s; m_row[m_Nnz] = i; m_col[m_Nnz] = j;
	++m_Nnz;
}
...
inline void McooQdoub::set(Qdoub_I s, Long_I i, Long_I j)
{
	...
	// push
	if (m_Nnz == m_N)
	    SLS_ERR("McooQdoub::set(): out of memory, please reserve! m_N = " + to_string(m_N));
	m_p[m_Nnz] = s; m_row[m_Nnz] = i; m_col[m_Nnz] = j;
	++m_Nnz;
}
...
void veccpy(Qcomp *, const Qcomp *, Long);

class McooQcomp : public VbaseQcomp
{
private:
...
#endif
	if (m_Nnz == m_N)
	    SLS_ERR("McooQcomp::push(): out of memory, please reserve! m_N = " + to_string(m_N));
	m_p[m_Nnz] = s; m_row[m_Nnz] = i; m_col[m_Nnz] = j;
	++m_Nnz;
}
...
inline void McooQcomp::set(Qcomp_I s, Long_I i, Long_I j)
{
	...
	// push
	if (m_Nnz == m_N)
	    SLS_ERR("McooQcomp::set(): out of memory, please reserve! m_N = " + to_string(m_N));
	m_p[m_Nnz] = s; m_row[m_Nnz] = i; m_col[m_Nnz] = j;
	++m_Nnz;
}
