...
RUN	cd ~/ && \
	...
	git clone https://github.com/MacroUniverse/SLISC-libs-x64-ubuntu18.04 --depth 1

ARG INSTALL_DIR=/home/$DOCKER_USER/SLISC-libs-x64-ubuntu18.04

# set number of threads for compilation
ARG NCPU=8

# ======== SLISC 32-bit dynamic no-quadmath ========
RUN cd ~/SLISC-libs-x64-ubuntu18.04 && source setup.sh && \
	cd ~/SLISC && \
