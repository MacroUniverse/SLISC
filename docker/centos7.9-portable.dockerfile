...
RUN yum install -y git vim time \
	soci-sqlite3-devel boost-filesystem wget mlocate automake libtool lzip
...
RUN source ~/SLISC0/make/ld_path_append.sh /usr/local/lib/ && \
	...
	export LIBRARY_PATH=$LIBRARY_PATH:$INSTALL_DIR/lapack32-so/lib64/ && \
	export LD_LIBRARY_PATH=$LD_LIBRARY_PATH:$INSTALL_DIR/lapack32-so/lib64/ && \
	sh bootstrap && ./configure --prefix=$INSTALL_DIR/arpack && \
	make -j12 && make check -j12 && make install
