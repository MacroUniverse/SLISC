...
RUN cd /root/arb && git checkout 2.19.0 && ./configure && make -j`getconf _NPROCESSORS_ONLN` && make install && \
	ln -s libarb.so /usr/local/lib/libflint-arb.so && \
	ldconfig

RUN cd /root/SLISC0 && touch SLISC/*.h && make -j`getconf _NPROCESSORS_ONLN` && \
