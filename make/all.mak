...
$(shell echo "// this file is auto generated by Makefile" > SLISC/config.h.new)
...
ifeq ($(opt_debug), true)
    ...
    $(info Build: Release)
    $(shell echo "#define NDEBUG" >> SLISC/config.h.new)
    release_flag = -O3
	debug_flag =
endif
...
ifeq ($(opt_main), true)
    $(info main.x: on)
    $(shell echo "#define SLS_TEST_ALL" >> SLISC/config.h.new)
else
    $(info main.x: off)
endif
...
ifeq ($(opt_asan), true)
    ifeq ($(opt_compiler), g++)
        ifeq ($(opt_static), false)
            ...
            $(shell echo "#define SLS_USE_ASAN" >> SLISC/config.h.new)
        else
...
ifeq ($(opt_long32), true)
    $(info Long bits: 32)
    $(shell echo "#define SLS_USE_INT_AS_LONG" >> SLISC/config.h.new)
else
    $(info Long bits: 64)
endif
...
# === CBLAS (reference) ===
    $(shell echo "#define SLS_USE_CBLAS" >> SLISC/config.h.new)
...
# === LAPACKE (reference) ===
    $(shell echo "#define SLS_USE_LAPACKE" >> SLISC/config.h.new)
    ifeq ($(opt_long32), false)
...
ifeq ($(opt_lapack), mkl)
    ...
    ifeq ($(opt_long32), true)
        ...
        ifeq ($(opt_compiler), g++)
            $(info Compiler: g++)
            $(shell echo "#define SLS_USE_MKL" >> SLISC/config.h.new)
    ...
    # Interface: 64bit int
        ...
        ifeq ($(opt_compiler), g++)
            $(info Compiler: g++)
            $(shell echo "#define SLS_USE_MKL" >> SLISC/config.h.new)
            $(shell echo "#define MKL_ILP64" >> SLISC/config.h.new)
            mkl_flag = -m64 # -I "${MKLROOT}/include"
            # static link
            ifeq ($(opt_static), true)
                $(info Link: static)
...
# === Boost ===
ifeq ($(opt_boost), true)
    $(shell echo "#define SLS_USE_BOOST" >> SLISC/config.h.new)
    ifeq ($(opt_static), false)
        $(info Boost: dynamic)
...
# === GSL ===
ifeq ($(opt_gsl), true)
    $(shell echo "#define SLS_USE_GSL" >> SLISC/config.h.new)
    ifeq ($(opt_static), false)
        $(info GSL: dynamic)
        gsl_lib = -l:libgsl.so
    else
...
ifeq ($(opt_eigen), true)
    $(info Eigen: on)
    $(shell echo "#define SLS_USE_EIGEN" >> SLISC/config.h.new)
else
    $(info Eigen: off)
endif
...
ifeq ($(opt_quadmath), true)
    ...
    $(shell echo "#define SLS_USE_QUAD_MATH" >> SLISC/config.h.new)
else
    $(info Quad Math: off)
	quad_math_flag =
endif
...
# === Arb ===
ifeq ($(opt_arb), true)
    $(shell echo "#define SLS_USE_ARB" >> SLISC/config.h.new)
    ifeq ($(opt_static), false)
        $(info Arb: dynamic)
...
ifeq ($(opt_mplapack), true)
    $(shell echo "#define SLS_USE_MPLAPACK" >> SLISC/config.h.new)
    ifeq ($(opt_static), false)
...
ifeq ($(opt_arpack), true)
    ifeq ($(opt_lapack), reference)
        $(shell echo "#define SLS_USE_ARPACK" >> SLISC/config.h.new)
        ifeq ($(opt_static), false)
            $(info Arpack: dynamic)
            ifeq ($(opt_long32), true)
...
ifeq ($(opt_sqlitecpp), true)
    opt_sqlite = false
    $(shell echo "#define SLS_USE_SQLITECPP" >> SLISC/config.h.new)
    $(shell echo "#define SLS_USE_SQLITE" >> SLISC/config.h.new)
...
ifeq ($(opt_matfile), true)
    ifeq ($(opt_static), false)
        ...
        $(shell echo "#define SLS_USE_MATFILE" >> SLISC/config.h.new)
