...
$(shell echo "// this file is auto generated by Makefile" > SLISC/config.h)
...
ifeq ($(opt_asan), true)
    ifeq ($(opt_compiler), g++)
        ifeq ($(opt_static), false)
            ...
            $(shell echo "#define SLS_USE_ASAN" >> SLISC/config.h)
        else
...
ifeq ($(opt_long32), true)
    $(info Long bits: 32)
    $(shell echo "#define SLS_USE_INT_AS_LONG" >> SLISC/config.h)
else
    $(info Long bits: 64)
endif
...
# === CBLAS (reference) ===
    $(shell echo "#define SLS_USE_CBLAS" >> SLISC/config.h)
...
# === LAPACKE (reference) ===
    $(shell echo "#define SLS_USE_LAPACKE" >> SLISC/config.h)
    ifeq ($(opt_long32), false)
...
# === Boost ===
ifeq ($(opt_boost), true)
    $(shell echo "#define SLS_USE_BOOST" >> SLISC/config.h)
    ifeq ($(opt_static), false)
        $(info Boost: dynamic)
...
# === GSL ===
ifeq ($(opt_gsl), true)
    $(shell echo "#define SLS_USE_GSL" >> SLISC/config.h)
    ifeq ($(opt_static), false)
        $(info GSL: dynamic)
        gsl_lib = -l:libgsl.so
    else
...
ifeq ($(opt_eigen), true)
    $(info Eigen: on)
    $(shell echo "#define SLS_USE_EIGEN" >> SLISC/config.h)
else
    $(info Eigen: off)
endif
...
ifeq ($(opt_quadmath), true)
    ...
    $(shell echo "#define SLS_USE_QUAD_MATH" >> SLISC/config.h)
else
    $(info Quad Math: off)
endif
...
# === Arb ===
ifeq ($(opt_arb), true)
    $(shell echo "#define SLS_USE_ARB" >> SLISC/config.h)
    ifeq ($(opt_static), false)
        $(info Arb: dynamic)
...
ifeq ($(opt_mplapack), true)
    $(shell echo "#define SLS_USE_MPLAPACK" >> SLISC/config.h)
    ifeq ($(opt_static), false)
...
ifeq ($(opt_arpack), true)
    ifeq ($(opt_lapack), reference)
        $(shell echo "#define SLS_USE_ARPACK" >> SLISC/config.h)
        ifeq ($(opt_static), false)
            $(info Arpack: dynamic)
            ifeq ($(opt_long32), true)
...
ifeq ($(opt_sqlitecpp), true)
    opt_sqlite = false
    $(shell echo "#define SLS_USE_SQLITECPP" >> SLISC/config.h)
    $(shell echo "#define SLS_USE_SQLITE" >> SLISC/config.h)
