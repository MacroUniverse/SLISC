...
tmp := $(shell echo "$(mydefine) SLS_MAJOR ${ver_major}" >> SLISC/config.h.new)
tmp := $(shell echo "$(mydefine) SLS_MINOR ${ver_minor}" >> SLISC/config.h.new)
tmp := $(shell echo "$(mydefine) SLS_PATCH ${ver_patch}" >> SLISC/config.h.new)
...
ifeq ($(opt_asan), true)
    ifeq ($(opt_compiler), g++)
        ifeq ($(opt_static), false)
            ...
            tmp := $(shell echo "$(mydefine) SLS_USE_ASAN" >> SLISC/config.h.new)
        else
...
ifeq ($(opt_debug), true)
    ...
    $(info Build: Release)
    tmp := $(shell echo "$(mydefine) NDEBUG" >> SLISC/config.h.new)
    release_flag := -O3
    debug_flag :=
endif
...
ifeq ($(opt_main), true)
    $(info main.x: on)
    tmp := $(shell echo "$(mydefine) SLS_TEST_ALL" >> SLISC/config.h.new)
else
    $(info main.x: off)
endif
...
ifeq ($(opt_long32), true)
    $(info Long bits: 32)
    tmp := $(shell echo "$(mydefine) SLS_USE_INT_AS_LONG" >> SLISC/config.h.new)
else
    $(info Long bits: 64)
endif
...
ifeq ($(opt_eigen), true)
    $(info Eigen: on)
    tmp := $(shell echo "$(mydefine) SLS_USE_EIGEN" >> SLISC/config.h.new)
else
    $(info Eigen: off)
endif
...
ifeq ($(opt_quadmath), true)
    $(info Quad Math: on)
    tmp := $(shell echo "$(mydefine) SLS_USE_QUAD_MATH" >> SLISC/config.h.new)
...
# === Arb ===
ifeq ($(opt_arb), true)
    tmp := $(shell echo "$(mydefine) SLS_USE_ARB" >> SLISC/config.h.new)
    ifeq ($(opt_static), false)
        $(info Arb: dynamic)
...
# === HDF5 ===
ifeq ($(opt_hdf), true)
    tmp := $(shell echo "$(mydefine) SLS_USE_HDF" >> SLISC/config.h.new)
    $(info HDF5: on)
...
ifeq ($(opt_matfile), true)
    ifeq ($(opt_static), false)
        ...
        tmp := $(shell echo "$(mydefine) SLS_USE_MATFILE" >> SLISC/config.h.new)
...
tmp := $(shell echo "$(mydefine) SLS_FP_EXCEPT" >> SLISC/config.h.new)
tmp := $(shell echo "$(mydefine) SLS_USE_UTFCPP" >> SLISC/config.h.new)
...
tmp := $(shell find . -maxdepth 1 -type d ! -path .)
tmp2 := $(patsubst ./%,../%/:,$(tmp))
tmp3 := $(shell echo '$(tmp2)' | sed 's/ //g')
in_paths := $(shell echo '$(tmp3)' | sed 's/.$$//')
