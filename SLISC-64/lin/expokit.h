...
// 'Comp', 'McooDoub', 'Comp';
// 'Comp', 'McooComp', 'Comp';
// inline void expokit_mul(@Ty@ *y, @Ta@_I a, @Tx@ *x)
...
// the callable `void mat_mul(Comp *y, const Comp *x)` performs matrix-vector multiplication for an N*N matrix
template <class Tmul>
inline void expv(SvecComp_IO v, Tmul &mat_mul, Doub_I t, Long_I Nkrylov, Doub_I mat_norm, SvecComp_IO wsp_c, SvecLong_IO wsp_i, Doub_I tol = 0, Bool_I her = false)
{
#ifdef SLS_CHECK_SHAPES
	if (wsp_c.size() < max(Long(10), v.size()*(Nkrylov + 2) + 5*sqr(Nkrylov + 2) + 8))
		SLS_ERR("wrong shape!");
...
{
	thread_local static VecComp wsp_c(max(Long(10), v.size()*(Nkrylov + 2) + 5 * sqr(Nkrylov + 2) + 8));
	thread_local static VecLong wsp_i(max(Nkrylov + 2, 7));
	SvecComp s_wsp_c(wsp_c.p(), wsp_c.size());
	SvecLong s_wsp_i(wsp_i.p(), wsp_i.size());
	expv(v, mat_mul, t, Nkrylov, mat_norm, s_wsp_c, s_wsp_i, tol, her);
}

template <class Tmul>
inline void expv(VecComp_IO v, Tmul &mat_mul, Doub_I t, Long_I Nkrylov, Doub_I mat_norm, SvecComp_IO wsp_c, SvecLong_IO wsp_i, Doub_I tol = 0, Bool_I her = false)
{
#ifdef SLS_CHECK_SHAPES
	if (wsp_c.size() < max(Long(10), v.size()*(Nkrylov + 2) + 5*sqr(Nkrylov + 2) + 8))
		SLS_ERR("wrong shape!");
