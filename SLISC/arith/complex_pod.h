...
5. operator std::complex<_Tp>() will convert to std::complex, this is only a host function
*/
...
template<class _Tp> complex<_Tp> CUDA_CALLABLE_MEMBER operator*(const complex<_Tp>& __z, const complex<_Tp>& __w);
...
public:
    complex() = default;
    CUDA_CALLABLE_MEMBER complex(const value_type& __re)
        : __re_(__re) {}
    CUDA_CALLABLE_MEMBER
    complex(const value_type& __re, const value_type& __im)
        : __re_(__re), __im_(__im) {}
    template<class _Xp> CUDA_CALLABLE_MEMBER
    complex(const complex<_Xp>& __c)
        : __re_(__c.real()), __im_(__c.imag()) {}
	CUDA_CALLABLE_MEMBER operator std::complex<_Tp>() { return std::complex<_Tp>(__re_, __im_); }
...
public:
    ...
    complex() = default;
    CUDA_CALLABLE_MEMBER complex(float __re)
        : __re_(__re) {}
	...
	operator std::complex<float>() { return std::complex<float>(__re_, __im_); }
...
public:
	...
	operator std::complex<double>() { return std::complex<double>(__re_, __im_); }
...
tanh(const complex<_Tp>& __x)
{
    ...
    return  complex<_Tp>(sinh(__2r)/__d, std::sin(__2i)/__d);
}
