...
// H is a hermitian matrix
// return error
inline Doub exp_miHdt_v_lanc(VecComp_IO v, CmatDoub_I H, Doub_I dt, Long_I Nkrylov, VecDoub_IO wsp_d, VecComp_IO wsp_c)
{
    Long N = v.size(), Nk = Nkrylov;
#ifdef SLS_CHECK_SHAPES
    if (Nk > N)
        SLS_ERR("Nk > N !");
    if (wsp_d.size() < Nk*Nk + 2*Nk)
...
inline Doub exp_miHdt_v_lanc(DvecComp_IO v, CmatDoub_I H, Doub_I dt, Long_I Nkrylov, VecDoub_IO wsp_d, VecComp_IO wsp_c)
{
    Long N = v.size(), Nk = Nkrylov;
#ifdef SLS_CHECK_SHAPES
    if (Nk > N)
...
inline Doub exp_miHdt_v_lanc(DvecComp_IO v, CmobdDoub_I H, Doub_I dt, Long_I Nkrylov, VecDoub_IO wsp_d, VecComp_IO wsp_c)
{
    Long N = v.size(), Nk = Nkrylov;
#ifdef SLS_CHECK_SHAPES
    if (Nk > N)
...
inline Doub exp_miHdt_v_lanc(DvecComp_IO v, CbandDoub_I H, Doub_I dt, Long_I Nkrylov, VecDoub_IO wsp_d, VecComp_IO wsp_c)
{
    Long N = v.size(), Nk = Nkrylov;
#ifdef SLS_CHECK_SHAPES
    if (Nk > N)
