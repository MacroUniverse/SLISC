...
inline void update_sqlite_table_query(
	...
	for (Long i = 0; i < size(field_names); ++i)
		q_insert << "?, ";
	q_insert.resize(q_insert.size()-3);
	q_insert << ");";
...
inline void update_sqlite_table(
	...
	while (stmt_select.executeStep()) {
		...
		if (get<0>(vals) != (Int)stmt_select.getColumn(0))
			changed = true;
		else if (get<1>(vals) != (Str)stmt_select.getColumn(1).getString())
			changed = true;
		else if (get<2>(vals) != (Str)stmt_select.getColumn(2).getString())
			changed = true;
		else if (get<3>(vals) != (Str)stmt_select.getColumn(3).getString())
			changed = true;
		if (changed) {
			// update db record
