...
inline Comp ODE_integration::extrapolation_in_zero (const unsigned int n,const Comp T[]) const
{  
  Comp f_in_zero = 0.0;
...
inline Comp ODE_integration::F_r_u (const Comp &z,const Comp &u) const
{
					   ...
					   const Comp &r,Comp &u,Comp &du) const
{
  ...
  Comp delta = h*(du0 + half_h*F_r_u (r0,u0));
  u = u0 + delta;
				  ...
				  const Comp &r,Comp &u,Comp &du) const
{
  ...
  Comp r_debut = r0,u_debut = u0,du_debut = du0,H = r-r0,u_end[8],du_end[8],u_extrapolated_next,du_extrapolated_next;
  double test = 1.0;
  ...
  while (test > precision)
  {
	...
	{
	  const Comp r_debut_plus_H = r_debut + H, r_end = (inf_norm (r - r_debut_plus_H) > inf_norm_half_H) ? (r_debut_plus_H) : (r);
	  ...
	  Comp u_extrapolated = extrapolation_in_zero (2,u_end); 
...
public:
  ...
  Coulomb_wave_functions (const bool is_it_normalized_c,const Comp &l_c,const Comp &eta_c)
	: l (l_c),
	  ...
	  log_sym_constant_arg_neg ((is_it_normalized_c) ? (-C_PI*(eta_c+(l_c+1)*Comp (0.0,1.0))) : (-C_PI *(l_c+1)*Comp (0.0,1.0))),
	  log_sym_constant_arg_pos ((is_it_normalized_c) ? (-C_PI*(eta_c-(l_c+1)*Comp (0.0,1.0))) : (C_PI*(l_c+1)*Comp (0.0,1.0))),
	  sym_constant_arg_neg ((is_it_normalized_c) ? (exp (-C_PI *(eta_c+(l_c+1)*Comp (0.0,1.0)))) : (exp (-C_PI*(l_c+1)*Comp (0.0,1.0)))),
	  sym_constant_arg_pos ((is_it_normalized_c) ? (exp (-C_PI*(eta_c-(l_c+1)*Comp (0.0,1.0)))) : (exp (C_PI*(l_c+1)*Comp (0.0,1.0)))), 
  ...
  void F_dF_init (const Comp &z,const Comp &F,const Comp &dF);

  void F_dF (const Comp &z,Comp &F,Comp &dF); 
  void G_dG (const Comp &z,Comp &G,Comp &dG);
  void H_dH (const int omega,const Comp &z,Comp &H,Comp &dH);
  void H_dH_scaled (const int omega,const Comp &z,Comp &H,Comp &dH);

  const Comp l,eta; // Angular momentum and Sommerfeld parameter.
...
private:
  ...
  void asymptotic_series (const int omega,const Comp &one_over_z,Comp sum[],Comp dsum[],bool &is_it_successful);

  Comp continued_fraction_f (const Comp &z,const int omega);
  Comp continued_fraction_h (const Comp &z,const int omega);

  void F_dF_power_series (const Comp &z,Comp &F,Comp &dF);
  ...
  void first_order_expansions (const bool is_it_regular,const Comp &z,Comp &B,Comp &dB);
  void H_dH_from_first_order_expansions (const int omega,const Comp &z,Comp &H,Comp &dH);

  void H_dH_with_F_dF_and_CF (const int omega,const Comp &z,Comp &H,Comp &dH);
  void H_dH_with_expansion (const int omega,const Comp &z,Comp &H,Comp &dH,bool &is_it_successful);

  void F_dF_with_symmetry_relations (const Comp &z,Comp &F,Comp &dF);
  ...
  const Comp log_Cl_eta,Cl_eta,sigma_l; // log[C(l,eta)], C(l,eta), sigma(l,eta)

  const Comp cut_constant_CFa_plus,cut_constant_CFa_minus,cut_constant_CFb_plus,cut_constant_CFb_minus;
  const Comp log_cut_constant_CFa_plus,log_cut_constant_CFa_minus,log_cut_constant_CFb_plus,log_cut_constant_CFb_minus;
  const Comp log_cut_constant_AS_plus,log_cut_constant_AS_minus;
  ...
  const Comp exp_I_chi,exp_minus_I_chi,one_over_sin_chi; 
  ...
  const Comp sym_constant_arg_neg,sym_constant_arg_pos,log_sym_constant_arg_neg,log_sym_constant_arg_pos;
