...
#ifndef NDEBUG
	if (beta[0] == 0)
		SLS_ERR("exp_Hdt_v_lanc(): input x is 0!");
#endif
	times(v0, x, 1/beta[0]); 
...
#ifndef NDEBUG
	if (beta[1] == 0)
		SLS_ERR("exp_Hdt_v_lanc(): input H*x is 0!");
#endif
	v1 /= beta[1];
	mul_fun(vc, v1);
...
#ifndef NDEBUG
		if (beta[j2] == 0)
			SLS_ERR("exp_Hdt_v_lanc(): input H^" + num2str(j2) + "*x is 0!");
#endif
		v2 /= beta[j2];
		mul_fun(vc, v2);
...
#ifndef NDEBUG
	if (beta[0] == 0)
		SLS_ERR("exp_miHdt_v_lanc(): input x is 0!");
#endif
	times(v0, x, 1/beta[0]);
	mul_fun(vc, v0);
	alpha[0] = real(dot(v0, vc));
...
#ifndef NDEBUG
	if (beta[1] == 0)
		SLS_ERR("exp_miHdt_v_lanc(): H*x is 0!");
#endif
	v1 /= beta[1];
	mul_fun(vc, v1);
	alpha[1] = real(dot(v1, vc));
...
#ifndef NDEBUG
		if (beta[j2] == 0)
			SLS_ERR("exp_miHdt_v_lanc(): H^" + num2str(j2) + "*x is 0!");
#endif
		v2 /= beta[j2];
		mul_fun(vc, v2);
		alpha[j2] = real(dot(v2, vc));
